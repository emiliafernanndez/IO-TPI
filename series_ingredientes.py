...
def plot_weekly_ingredient_series(weekly_ingredient_path, output_folder='ingredient_time_series'):
    # Cargar datos semanales de insumos
    weekly_ingredients = pd.read_csv(weekly_ingredient_path, index_col=0, parse_dates=True)
...
def generate_weekly_ingredient_series(sales_data_path, ingredient_data_path, selected_ingredients, output_path):
    ...
    ingredient_data = pd.read_csv(ingredient_data_path, index_col=0)
    ...
    for product, group in sales_data.groupby('article'):
        if product in ingredient_data.index:
            ...
            for week, week_data in group.groupby('week'):
                weekly_ingredients.loc[week] += (week_data['Quantity'].sum() * product_ingredients)
...
selected_ingredients=['Harina de Trigo (g)','Manteca (g)','Sal (g)','Az̼car (g)']
...
plot_weekly_ingredient_series('weekly_ingredients.csv')
